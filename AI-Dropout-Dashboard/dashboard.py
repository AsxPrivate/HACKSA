...
# -----------------------
st.sidebar.header("Daily Attendance Heatmap")
daily_attendance_file = st.sidebar.file_uploader("Upload daily attendance spreadsheet", type=["csv", "xlsx"])

daily_attendance_df = None
if daily_attendance_file:
    try:
        if daily_attendance_file.name.endswith(".csv"):
            daily_attendance_df = pd.read_csv(daily_attendance_file)
        else:
            daily_attendance_df = pd.read_excel(daily_attendance_file)
    except Exception as e:
        st.warning(f"Could not read daily attendance file: {e}")

if daily_attendance_df is not None:
    st.subheader("Daily Attendance Heatmap")
    # Expect columns: student_id, date, attendance_percent or present/absent
    # If present/absent, convert to 1/0
    if "attendance_percent" in daily_attendance_df.columns:
        pivot_df = daily_attendance_df.pivot(index="student_id", columns="date", values="attendance_percent")
    elif "present" in daily_attendance_df.columns:
        # Assume present is 1/0 or "Yes"/"No"
        daily_attendance_df["present"] = daily_attendance_df["present"].replace({"Yes":1,"No":0})
        pivot_df = daily_attendance_df.pivot(index="student_id", columns="date", values="present")
    else:
        st.info("Spreadsheet must have either 'attendance_percent' or 'present' column.")
        pivot_df = None
    ...
    if pivot_df is not None:
        ...
        st.caption("Rows: Students, Columns: Dates. Color = attendance % or present/absent.")
        fig, ax = plt.subplots(figsize=(min(12, 0.5+0.3*len(pivot_df.columns)), min(8, 0.5+0.2*len(pivot_df))))
        sns.heatmap(pivot_df, cmap="YlGnBu", ax=ax, cbar=True)
        ax.set_xlabel("Date")
